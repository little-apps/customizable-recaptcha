...
define('WP_GRECAPTCHA_PATH', plugin_dir_path(__FILE__));
...
class WPGRecaptcha {
	...
	protected $required_fields = array(
		...
		'comments' => array('{recaptcha_class}', '{recaptcha_public_key}', '{recaptcha_theme}', '{recaptcha_js}', '{error_recaptcha_code}'),
...
{recaptcha_js}',
			'comments_recaptcha_code' => '{error_recaptcha_code}
...
{recaptcha_js}',
	...
	public function check_recaptcha_comments_post_redirect($location, $comment) {
        if (isset($this->captcha_error) && $this->captcha_error) {
            $location = add_query_arg(array('comment-id' => $comment->comment_ID, 'captcha' => 'failed'), $location);
        }
...
?>
    ...
    public function validate($input) {
		...
		foreach (array('register', 'login', 'lostpassword', 'comments', 'error') as $form) {
		...
		if (!empty($input[$setting_code])) {
			...
			foreach ($this->required_fields[$form] as $field) {
				if (strpos($input[$setting_code], $field) === false) {
					$message = 'The code for "'.$form.'" is missing one of the following fields: '.implode(',', $required_fields).'. ';
